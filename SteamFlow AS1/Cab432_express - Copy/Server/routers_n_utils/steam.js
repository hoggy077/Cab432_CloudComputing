...
exports.paths = {
    ...
    //Listed Backend
    //applist: async (parameters) => {
    //    //not required anymore, faster to validate on a per request
    //    var r = null;
    //    await axios.get(`${Steam_BaseListedAPI}ISteamApps/GetAppList/v2/`).then((response) => {
    //        r = response;
    //        r.status = response.status;
    //    }).catch((error) => {
    //        r = error;
...
router.get("/api/v1/steam/wishlist/:steamid", async (req, res) => {
    try {
        var r = {};
        if (typeof req.params.steamid == undefined || req.params.steamid < 0) {
            r = { error: "SteamID has been excluded from URL", api: "Steam" }
            res.status(400).json(r);
            return;
        }

        //this will only get 1 page of wishlist. The quantity per page is unknown or else I would poll through each page for the complete list
        await axios.get(`${Steam_BaseUnlistedAPI}wishlist/profiles/${req.params.steamid}/wishlistdata`).then((response) => {
            r = response;
            r.status = response.status;
        }).catch((error) => {
            r = error;
        ...
        if (r.status != 200) {
            ...
            res.status(r.status).json({ error: "Steam account couldn't be accessed. Please ensure your account and game details are public and try again.", api: "Steam" });
        }
        else {
            //reached it
            //console.log(r);
